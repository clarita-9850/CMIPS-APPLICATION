...
export default function FieldMaskingTab() {
  ...
  const { data: interfaceData, isLoading: isInterfaceLoading } = useQuery({
    ...
    enabled: !!selectedRole && !!selectedReportType,
  });
  ...
  return (
    <div>
      <div className="mb-4">
        <h2 className="card-title mb-1">Field Masking Configuration</h2>
        <p className="text-muted">Configure field masking rules for different user roles and report types</p>
      </div>
      ...
      {isInterfaceLoading || isLoadingFields ? (
        <div className="text-center py-4">
          <p className="text-muted">Loading field masking rules...</p>
        </div>
      ) : localRules.length > 0 ? (
        <div className="card">
          <div className="card-header" style={{ backgroundColor: 'var(--color-p2, #046b99)', color: 'white' }}>
            <h3 className="card-title mb-0" style={{ color: 'white' }}>Field Masking Rules</h3>
          </div>

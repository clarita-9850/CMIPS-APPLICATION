...
const NAV_LINKS = [
    { label: 'list Create Zip Code', route: '/misc/user-list-create-zip-code' }
  ];

export function UserRemoveZipCodePage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('misc', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"User_removeZipCode"}
...
export default UserRemoveZipCodePage;

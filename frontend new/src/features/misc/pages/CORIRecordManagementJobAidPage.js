...
export function CORIRecordManagementJobAidPage() {
  const { data, loading, error } = useDomainData('misc', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"CORI_Record_Management_Job_Aid"}
      title={"Record Management Job Aid"}
      navLinks={NAV_LINKS}
...
export default CORIRecordManagementJobAidPage;

...
export function TimesheetRejectRequestConfirmPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('misc', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Timesheet_rejectRequestConfirm"}
      title={"Reject Request Confirm"}
      navLinks={NAV_LINKS}
...
export default TimesheetRejectRequestConfirmPage;

...
export function GenericViewEvidenceTransactionPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('misc', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Generic_viewEvidenceTransaction"}
    ...
    >
      ...
      <UimSection title={"Details"}>
        <div className="uim-form-grid">
          ...
          <UimField label={"Event Description"} value={record && record['eventDescription']} />
...
export default GenericViewEvidenceTransactionPage;

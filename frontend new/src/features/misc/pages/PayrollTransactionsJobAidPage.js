...
export function PayrollTransactionsJobAidPage() {
  const { data, loading, error } = useDomainData('misc', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Payroll_Transactions_Job_Aid"}
      title={"Transactions Job Aid"}
      navLinks={NAV_LINKS}
...
export default PayrollTransactionsJobAidPage;

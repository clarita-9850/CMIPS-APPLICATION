...
export function InterCountyTransfersJobAidPage() {
  const { data, loading, error } = useDomainData('misc', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Inter_County_Transfers_Job_Aid"}
      title={"County Transfers Job Aid"}
      navLinks={NAV_LINKS}
...
export default InterCountyTransfersJobAidPage;

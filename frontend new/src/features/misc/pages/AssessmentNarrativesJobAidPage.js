...
export function AssessmentNarrativesJobAidPage() {
  const { data, loading, error } = useDomainData('misc', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Assessment_Narratives_Job_Aid"}
      title={"Narratives Job Aid"}
      navLinks={NAV_LINKS}
...
export default AssessmentNarrativesJobAidPage;

...
export function ClosedReferralConfirmationPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('misc', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"ClosedReferral_confirmation"}
      title={"Change Open Referral Type To Closed Referral:"}
      navLinks={NAV_LINKS}
...
export default ClosedReferralConfirmationPage;

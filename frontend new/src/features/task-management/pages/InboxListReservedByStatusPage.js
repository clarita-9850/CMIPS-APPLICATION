...
export function InboxListReservedByStatusPage() {
  const { data, loading, error } = useDomainData('task-management', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Inbox_listReservedByStatus"}
      title={"Reserved Tasks by Status:\\\\"}
      navLinks={NAV_LINKS}
...
export default InboxListReservedByStatusPage;

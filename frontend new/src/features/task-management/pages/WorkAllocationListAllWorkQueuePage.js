...
const NAV_LINKS = [
    { label: 'create Work Queue', route: '/task-management/create-work-queue' },
    { label: 'view Work Queue', route: '/task-management/view-work-queue' },
    { label: 'modify Work Queue From List', route: '/task-management/modify-work-queue-from-list' },
    { label: 'user Home', route: '/organization/user-home' }
  ];

export function WorkAllocationListAllWorkQueuePage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('task-management', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"WorkAllocation_listAllWorkQueue"}
      title={"Work Queues"}
      navLinks={NAV_LINKS}
    ...
    >
      ...
      <div className="uim-action-bar">
        ...
        <button className="uim-btn uim-btn-primary" onClick={() => navigate('/task-management/view-work-queue')}>View</button>
...
export default WorkAllocationListAllWorkQueuePage;

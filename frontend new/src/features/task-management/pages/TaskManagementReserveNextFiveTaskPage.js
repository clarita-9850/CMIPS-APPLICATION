...
export function TaskManagementReserveNextFiveTaskPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('task-management', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"TaskManagement_ReserveNextFiveTask"}
      title={"Reserve Next 5 Tasks:\\\\"}
      navLinks={NAV_LINKS}
...
export default TaskManagementReserveNextFiveTaskPage;

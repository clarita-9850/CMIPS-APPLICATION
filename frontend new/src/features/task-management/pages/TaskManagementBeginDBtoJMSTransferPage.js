...
export function TaskManagementBeginDBtoJMSTransferPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('task-management', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"TaskManagement_beginDBtoJMSTransfer"}
      title={"Begin D Bto J M S Transfer"}
      navLinks={NAV_LINKS}
...
export default TaskManagementBeginDBtoJMSTransferPage;

...
export function WorkAllocationListCDSSWPCSWorkQueuePage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('task-management', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"WorkAllocation_listCDSSWPCSWorkQueue"}
      title={"Work Queues"}
      navLinks={NAV_LINKS}
...
export default WorkAllocationListCDSSWPCSWorkQueuePage;

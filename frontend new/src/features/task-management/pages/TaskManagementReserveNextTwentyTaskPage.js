...
export function TaskManagementReserveNextTwentyTaskPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('task-management', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"TaskManagement_ReserveNextTwentyTask"}
      title={"Reserve Next 20 Tasks:\\\\"}
      navLinks={NAV_LINKS}
...
export default TaskManagementReserveNextTwentyTaskPage;

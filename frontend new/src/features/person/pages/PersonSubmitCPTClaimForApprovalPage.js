...
export function PersonSubmitCPTClaimForApprovalPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('person', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Person_submitCPTClaimForApproval"}
      title={"Submit Training Time Claim for Approval:"}
      navLinks={NAV_LINKS}
...
export default PersonSubmitCPTClaimForApprovalPage;

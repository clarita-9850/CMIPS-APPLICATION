...
export function PersonSearchTimesheetPage() {
  const personsApi = getDomainApi('person');
  const { data, loading, error } = useDomainData('person', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Person_searchTimesheet"}
    ...
    >
      ...
      <UimSection title={"Search Criteria"}>
        <div className="uim-form-grid">
          ...
          <UimField label={"Timesheet Status"} value={record && record['timesheetStatus']} />
          <UimField label={"Timesheet Number"} value={record && record['timesheetNumber']} />
...
export default PersonSearchTimesheetPage;

...
const NAV_LINKS = [
    ...
    { label: 'search Timesheet', route: '/person/search-timesheet' }
  ];

export function PersonViewTimesheetPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('person', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Person_viewTimesheet"}
    ...
    >
      ...
      <UimSection title={"Time Entries [HH:MM]"}>
        <div className="uim-form-grid">
          ...
          <UimField label={"Processed Through Telephonic Solution"} value={record && record['processedThroughTelephonicSolution']} />
...
export default PersonViewTimesheetPage;

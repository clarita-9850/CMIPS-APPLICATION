...
const NAV_LINKS = [
    { label: 'view Overtime Violation Exemption History E C', route: '/person/view-overtime-violation-exemption-history-ec' },
    { label: 'overtime Violation Exemption', route: '/person/overtime-violation-exemption' }
  ];

export function PersonOvertimeViolationExemptionHistoryECPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('person', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Person_overtimeViolationExemptionHistoryEC"}
    ...
    >
      ...
      <div className="uim-action-bar">
        <button className="uim-btn uim-btn-primary" onClick={() => navigate('/person/view-overtime-violation-exemption-history-ec')}>View</button>
...
export default PersonOvertimeViolationExemptionHistoryECPage;

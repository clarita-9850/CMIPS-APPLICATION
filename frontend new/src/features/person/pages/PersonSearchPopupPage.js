...
export function PersonSearchPopupPage() {
  const personsApi = getDomainApi('person');
  const { data, loading, error } = useDomainData('person', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Person_searchPopup"}
      title={"Person Search"}
      navLinks={NAV_LINKS}
    ...
    >
      ...
      <UimSection title={"General"}>
        <div className="uim-form-grid">
          ...
          <UimField label={"Unit Type"} value={record && record['unitType']} />
          <UimField label={"City"} value={record && record['city']} />
          <UimField label={"Street Name"} value={record && record['streetName']} />
          <UimField label={"Unit Number"} value={record && record['unitNumber']} />
      ...
      <UimSection title={"Address"}>
        <div className="uim-form-grid">
          ...
          <UimField label={"City"} value={record && record['city']} />
          <UimField label={"County"} value={record && record['county']} />
          <UimField label={"Provider Eligibility"} value={record && record['providerEligibility']} />
        </div>
      </UimSection>
...
export default PersonSearchPopupPage;

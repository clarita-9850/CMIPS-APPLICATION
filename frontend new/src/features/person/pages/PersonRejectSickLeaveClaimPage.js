...
export function PersonRejectSickLeaveClaimPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('person', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Person_rejectSickLeaveClaim"}
      title={"Reject Sick Leave Claim"}
      navLinks={NAV_LINKS}
...
export default PersonRejectSickLeaveClaimPage;

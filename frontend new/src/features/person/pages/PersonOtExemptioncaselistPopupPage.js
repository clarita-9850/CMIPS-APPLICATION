...
export function PersonOtExemptioncaselistPopupPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('person', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Person_otExemptioncaselistPopup"}
      title={"Select Recipient OT Exemption:"}
      navLinks={NAV_LINKS}
...
export default PersonOtExemptioncaselistPopupPage;

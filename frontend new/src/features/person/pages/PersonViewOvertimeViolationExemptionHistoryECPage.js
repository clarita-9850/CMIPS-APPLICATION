...
export function PersonViewOvertimeViolationExemptionHistoryECPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('person', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Person_viewOvertimeViolationExemptionHistoryEC"}
      title={"View Overtime Violation Exemption History - Extraordinary Circumstance:"}
      navLinks={NAV_LINKS}
...
export default PersonViewOvertimeViolationExemptionHistoryECPage;

...
export function PersonViewSpecTranPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('person', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Person_viewSpecTran"}
    ...
    >
      ...
      <UimSection title={"Options"}>
        <div className="uim-form-grid">
          ...
          <UimField label={"Approved By"} value={record && record['approvedBy']} />
...
export default PersonViewSpecTranPage;

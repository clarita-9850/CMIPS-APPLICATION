...
export function PersonViewOvertimeViolationExemptionHistoryPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('person', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Person_viewOvertimeViolationExemptionHistory"}
      title={"View Overtime Violation Exemption History:"}
      navLinks={NAV_LINKS}
...
export default PersonViewOvertimeViolationExemptionHistoryPage;

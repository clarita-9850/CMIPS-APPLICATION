...
export function PersonViewEVVTimsheetDetailsForHistoryPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('person', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Person_viewEVVTimsheetDetailsForHistory"}
      title={"EVV Details:\\\\"}
      navLinks={NAV_LINKS}
...
export default PersonViewEVVTimsheetDetailsForHistoryPage;

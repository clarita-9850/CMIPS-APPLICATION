...
export function PersonConfirmEVVHardCopyPrintRecipPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('person', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Person_confirmEVVHardCopyPrintRecip"}
      title={"Confirm E V V Hard Copy Print Recip"}
      navLinks={NAV_LINKS}
...
export default PersonConfirmEVVHardCopyPrintRecipPage;

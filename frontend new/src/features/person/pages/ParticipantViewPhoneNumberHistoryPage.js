...
export function ParticipantViewPhoneNumberHistoryPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('person', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Participant_viewPhoneNumberHistory"}
      title={"View Phone Number History:\\\\"}
      navLinks={NAV_LINKS}
...
export default ParticipantViewPhoneNumberHistoryPage;

...
export function PersonRestoreUploadedDocumentPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('person', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Person_restoreUploadedDocument"}
      title={"Restore Uploaded Document"}
      navLinks={NAV_LINKS}
...
export default PersonRestoreUploadedDocumentPage;

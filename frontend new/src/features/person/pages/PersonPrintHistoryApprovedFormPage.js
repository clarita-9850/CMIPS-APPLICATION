...
const NAV_LINKS = [
    { label: 'cancel Print Eform', route: '/person/cancel-print-eform' },
    { label: 'list Approved Electronic Forms', route: '/person/list-approved-electronic-forms' }
  ];

export function PersonPrintHistoryApprovedFormPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('person', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Person_printHistoryApprovedForm"}
      title={"Print History:"}
      navLinks={NAV_LINKS}
...
export default PersonPrintHistoryApprovedFormPage;

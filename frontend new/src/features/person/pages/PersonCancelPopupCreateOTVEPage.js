...
const NAV_LINKS = [
    ...
    { label: 'create Overtime Violation Exemption Extraordinaty Circumstance', route: '/person/create-overtime-violation-exemption-extraordinaty-circumstance' }
  ];

export function PersonCancelPopupCreateOTVEPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('person', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Person_cancelPopupCreateOTVE"}
      title={"Cancel Popup Create O T V E"}
      navLinks={NAV_LINKS}
...
export default PersonCancelPopupCreateOTVEPage;

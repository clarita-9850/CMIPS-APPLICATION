...
export function PersonArchiveUploadedDocumentPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('person', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Person_archiveUploadedDocument"}
      title={"Archive Uploaded Document"}
      navLinks={NAV_LINKS}
...
export default PersonArchiveUploadedDocumentPage;

...
export function PersonApproveElectronicFormsPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('person', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Person_approveElectronicForms"}
      title={"Approve Electronic Forms"}
      navLinks={NAV_LINKS}
...
export default PersonApproveElectronicFormsPage;

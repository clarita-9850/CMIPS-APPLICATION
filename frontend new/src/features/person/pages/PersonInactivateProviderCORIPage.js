...
export function PersonInactivateProviderCORIPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('person', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Person_inactivateProviderCORI"}
      title={"Inactivate Provider C O R I"}
      navLinks={NAV_LINKS}
...
export default PersonInactivateProviderCORIPage;

...
const NAV_LINKS = [
    ...
    { label: 'modify C P Training Claim', route: '/person/modify-cp-training-claim' },
    { label: 'search C P Claims', route: '/person/search-cp-claims' }
  ];

export function PersonViewCPTrainingClaimPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('person', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Person_viewCPTrainingClaim"}
    ...
    >
      ...
      <div className="uim-action-bar">
        ...
        <button className="uim-btn uim-btn-primary" onClick={() => navigate('/person/view-cp-claim-history')}>History</button>
...
export default PersonViewCPTrainingClaimPage;

...
export function PersonCreateProviderCORIPage() {
  const navigate = useNavigate();
  const personsApi = getDomainApi('person');
  return (
    <UimPageLayout
      pageId={"Person_createProviderCORI"}
      title={"Create Provider CORI"}
      navLinks={NAV_LINKS}
      hidePlaceholderBanner={true}
    >
      <UimSection title={"Provider CORI Details"}>
        <div className="uim-form-grid">
          <UimField label={"CORI Date"} />
          <UimField label={"Conviction or Release Date"} />
          <UimField label={"CORI End Date"} />
          <UimField label={"Tier"} />
        </div>
      </UimSection>
...
export default PersonCreateProviderCORIPage;

...
const NAV_LINKS = [
    ...
    { label: 'view Message Center', route: '/person/view-message-center' },
    { label: 'user Home Popup', route: '/organization/user-home-popup' }
  ];

export function PersonListMessageCenterPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('person', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Person_listMessageCenter"}
    ...
    >
      ...
      <div className="uim-action-bar">
        ...
        <button className="uim-btn uim-btn-primary" onClick={() => navigate('/person/view-message-center')}>View</button>
        <button className="uim-btn uim-btn-primary" onClick={() => alert('Action: &lt;&lt;Previous')}>&lt;&lt;Previous</button>
        <button className="uim-btn uim-btn-primary" onClick={() => alert('Action: Next&gt;&gt;')}>Next&gt;&gt;</button>
        <button className="uim-btn uim-btn-primary" onClick={() => navigate('/person/view-message-center')}>View</button>
      </div>
...
export default PersonListMessageCenterPage;

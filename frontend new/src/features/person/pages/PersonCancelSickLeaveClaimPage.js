...
export function PersonCancelSickLeaveClaimPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('person', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Person_CancelSickLeaveClaim"}
      title={"Confirm Sick Leave Claim Cancellation:"}
      navLinks={NAV_LINKS}
...
export default PersonCancelSickLeaveClaimPage;

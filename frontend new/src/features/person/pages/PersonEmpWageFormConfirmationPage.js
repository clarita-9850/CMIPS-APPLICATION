...
export function PersonEmpWageFormConfirmationPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('person', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Person_empWageFormConfirmation"}
      title={"Emp Wage Form Confirmation"}
      navLinks={NAV_LINKS}
...
export default PersonEmpWageFormConfirmationPage;

...
const NAV_LINKS = [
    { label: 'modify Email Address From View', route: '/person/modify-email-address-from-view' },
    { label: 'list Email Address', route: '/person/list-email-address' }
  ];

export function ParticipantViewEmailAddressPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('person', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Participant_viewEmailAddress"}
...
export default ParticipantViewEmailAddressPage;

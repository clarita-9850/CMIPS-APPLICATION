...
export function PersonApproveProviderEnrollmentPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('person', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Person_approveProviderEnrollment"}
      title={"Approve Provider Enrollment:"}
      navLinks={NAV_LINKS}
...
export default PersonApproveProviderEnrollmentPage;

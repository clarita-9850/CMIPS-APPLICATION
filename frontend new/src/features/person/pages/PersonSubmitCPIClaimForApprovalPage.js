...
export function PersonSubmitCPIClaimForApprovalPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('person', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Person_submitCPIClaimForApproval"}
      title={"Submit Incentive Claim for Approval:"}
      navLinks={NAV_LINKS}
...
export default PersonSubmitCPIClaimForApprovalPage;

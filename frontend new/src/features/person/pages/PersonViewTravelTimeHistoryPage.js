...
export function PersonViewTravelTimeHistoryPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('person', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Person_viewTravelTimeHistory"}
      title={"Travel Time  History:"}
      navLinks={NAV_LINKS}
...
export default PersonViewTravelTimeHistoryPage;

...
const NAV_LINKS = [
    ...
    { label: 'B V I Timesheet History', route: '/person/bvi-timesheet-history' }
  ];

export function PersonViewProviderETRejectReleasePage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('person', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Person_ViewProviderETRejectRelease"}
...
export default PersonViewProviderETRejectReleasePage;

...
export function PersonUndeleteUploadedDocumentPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('person', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Person_undeleteUploadedDocument"}
      title={"Undelete Uploaded Document"}
      navLinks={NAV_LINKS}
...
export default PersonUndeleteUploadedDocumentPage;

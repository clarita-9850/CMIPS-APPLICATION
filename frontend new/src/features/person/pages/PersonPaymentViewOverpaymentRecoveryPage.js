...
export function PersonPaymentViewOverpaymentRecoveryPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('person', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"PersonPayment_viewOverpaymentRecovery"}
...
export default PersonPaymentViewOverpaymentRecoveryPage;

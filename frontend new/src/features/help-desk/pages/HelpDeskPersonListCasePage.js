...
const NAV_LINKS = [
    { label: 'person Home', route: '/help-desk/person-home' },
    { label: 'user Home Popup', route: '/organization/user-home-popup' }
  ];

export function HelpDeskPersonListCasePage() {
  const { data, loading, error } = useDomainData('help-desk', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"HelpDesk_person_listCase"}
      title={"Cases:"}
      navLinks={NAV_LINKS}
    ...
    >
      ...
      <UimSection title={"Details"}>
        <div className="uim-form-grid">
          ...
          <UimField label={"Live-In Status"} value={record && record['liveInStatus']} />
          <UimField label={"Live-In Status Date"} value={record && record['liveInStatusDate']} />
        </div>
      </UimSection>
...
export default HelpDeskPersonListCasePage;

...
export function CaseSubmitapproveSpecialTransactionPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('case', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Case_submitapproveSpecialTransaction"}
      title={"Submit Special Transaction for Approval:"}
      navLinks={NAV_LINKS}
...
export default CaseSubmitapproveSpecialTransactionPage;

...
const NAV_LINKS = [
    { label: 'edit Special Transaction Travel', route: '/case/edit-special-transaction-travel' },
    { label: 'list Special Transactions', route: '/case/list-special-transactions' }
  ];

export function CaseViewSpecialTransactionTravelPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('case', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Case_viewSpecialTransactionTravel"}
...
export default CaseViewSpecialTransactionTravelPage;

...
export function CaseCancelForgedEndorsementAffidavitPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('case', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Case_cancelForgedEndorsementAffidavit"}
      title={"Cancel Forged Endorsement Affidavit"}
      navLinks={NAV_LINKS}
...
export default CaseCancelForgedEndorsementAffidavitPage;

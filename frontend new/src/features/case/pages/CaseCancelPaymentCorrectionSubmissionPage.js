...
export function CaseCancelPaymentCorrectionSubmissionPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('case', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Case_cancelPaymentCorrectionSubmission"}
      title={"Cancel Payment Correction Submission"}
      navLinks={NAV_LINKS}
...
export default CaseCancelPaymentCorrectionSubmissionPage;

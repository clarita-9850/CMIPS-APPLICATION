...
export function CaseReIssueSpecialTransactionPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('case', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Case_reIssueSpecialTransaction"}
      title={"Reissue Special Transaction:"}
      navLinks={NAV_LINKS}
...
export default CaseReIssueSpecialTransactionPage;

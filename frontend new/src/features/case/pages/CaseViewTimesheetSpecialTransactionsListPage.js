...
const NAV_LINKS = [
    { label: 'view Timesheet Special Transactions', route: '/case/view-timesheet-special-transactions' }
  ];

export function CaseViewTimesheetSpecialTransactionsListPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('case', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Case_viewTimesheetSpecialTransactionsList"}
      title={"View Special Transactions List:"}
      navLinks={NAV_LINKS}
...
export default CaseViewTimesheetSpecialTransactionsListPage;

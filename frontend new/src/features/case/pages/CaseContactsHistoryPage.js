...
const NAV_LINKS = [
    { label: 'view Contact History', route: '/case/view-contact-history' },
    { label: 'list Contacts', route: '/case/list-contacts' }
  ];

export function CaseContactsHistoryPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('case', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Case_contactsHistory"}
    ...
    >
      ...
      <div className="uim-action-bar">
        <button className="uim-btn uim-btn-primary" onClick={() => navigate('/case/view-contact-history')}>View</button>
...
export default CaseContactsHistoryPage;

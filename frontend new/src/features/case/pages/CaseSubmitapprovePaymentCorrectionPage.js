...
export function CaseSubmitapprovePaymentCorrectionPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('case', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Case_submitapprovePaymentCorrection"}
      title={"Submitapprove Payment Correction"}
      navLinks={NAV_LINKS}
...
export default CaseSubmitapprovePaymentCorrectionPage;

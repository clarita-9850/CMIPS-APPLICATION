...
export function CaseViewCommentFormsHistroyPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('case', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Case_viewCommentFormsHistroy"}
      title={"View Comment Forms Histroy"}
      navLinks={NAV_LINKS}
...
export default CaseViewCommentFormsHistroyPage;

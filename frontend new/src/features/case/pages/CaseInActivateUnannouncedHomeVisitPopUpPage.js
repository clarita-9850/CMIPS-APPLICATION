...
export function CaseInActivateUnannouncedHomeVisitPopUpPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('case', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Case_inActivateUnannouncedHomeVisitPopUp"}
      title={"Inactivate Unannounced Home Visit:"}
      navLinks={NAV_LINKS}
...
export default CaseInActivateUnannouncedHomeVisitPopUpPage;

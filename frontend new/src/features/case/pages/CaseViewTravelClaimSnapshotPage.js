...
const NAV_LINKS = [
    ...
    { label: 'travel Claim History', route: '/case/travel-claim-history' }
  ];

export function CaseViewTravelClaimSnapshotPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('case', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Case_viewTravelClaimSnapshot"}
    ...
    >
      ...
      <div className="uim-action-bar">
        <button className="uim-btn uim-btn-primary" onClick={() => navigate('/case/view-travel-claim')}>View Travel Claim Details</button>
        <button className="uim-btn uim-btn-primary" onClick={() => navigate('/case/travel-claim-history')}>History</button>
...
export default CaseViewTravelClaimSnapshotPage;

...
const NAV_LINKS = [
    { label: 'view Payment Correction Pop Up', route: '/case/view-payment-correction-pop-up' },
    { label: 'view Timesheet', route: '/case/view-timesheet' }
  ];

export function CaseViewPaymentCorrectionListPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('case', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Case_viewPaymentCorrectionList"}
...
export default CaseViewPaymentCorrectionListPage;

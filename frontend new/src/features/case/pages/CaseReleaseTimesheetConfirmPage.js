...
export function CaseReleaseTimesheetConfirmPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('case', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Case_releaseTimesheetConfirm"}
      title={"Release Timesheet Confirm"}
      navLinks={NAV_LINKS}
...
export default CaseReleaseTimesheetConfirmPage;

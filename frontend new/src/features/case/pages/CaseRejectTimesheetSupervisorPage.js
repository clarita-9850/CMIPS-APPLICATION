...
export function CaseRejectTimesheetSupervisorPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('case', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Case_rejectTimesheetSupervisor"}
    ...
    >
      ...
      <UimSection title={"Details"}>
        <div className="uim-form-grid">
          <UimField label={"Reject Approval Request"} value={record && record['rejectApprovalRequest']} />
...
export default CaseRejectTimesheetSupervisorPage;

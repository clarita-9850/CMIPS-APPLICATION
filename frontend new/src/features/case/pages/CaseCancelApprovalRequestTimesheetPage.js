...
export function CaseCancelApprovalRequestTimesheetPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('case', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Case_cancelApprovalRequestTimesheet"}
    ...
    >
      ...
      <UimSection title={"Details"}>
        <div className="uim-form-grid">
          <UimField label={"Cancel Request for Approval"} value={record && record['cancelRequestForApproval']} />
        </div>
      </UimSection>
...
export default CaseCancelApprovalRequestTimesheetPage;

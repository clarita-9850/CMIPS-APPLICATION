...
export function CaseConfirmEVVHardCopyPrintProvPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('case', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Case_confirmEVVHardCopyPrintProv"}
      title={"Confirm E V V Hard Copy Print Prov"}
      navLinks={NAV_LINKS}
...
export default CaseConfirmEVVHardCopyPrintProvPage;

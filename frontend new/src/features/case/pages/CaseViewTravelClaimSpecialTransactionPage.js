...
export function CaseViewTravelClaimSpecialTransactionPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('case', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Case_viewTravelClaimSpecialTransaction"}
...
export default CaseViewTravelClaimSpecialTransactionPage;

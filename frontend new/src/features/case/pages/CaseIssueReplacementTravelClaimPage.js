...
export function CaseIssueReplacementTravelClaimPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('case', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Case_issueReplacementTravelClaim"}
      title={"Travel Claim Issuance:"}
      navLinks={NAV_LINKS}
...
export default CaseIssueReplacementTravelClaimPage;

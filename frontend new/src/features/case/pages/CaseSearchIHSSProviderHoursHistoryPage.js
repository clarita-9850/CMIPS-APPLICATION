...
export function CaseSearchIHSSProviderHoursHistoryPage() {
  const casesApi = getDomainApi('case');
  const { data, loading, error } = useDomainData('case', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Case_searchIHSSProviderHoursHistory"}
      title={"View IHSS Provider Hours History:"}
      navLinks={NAV_LINKS}
...
export default CaseSearchIHSSProviderHoursHistoryPage;

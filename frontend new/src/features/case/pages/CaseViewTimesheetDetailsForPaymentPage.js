...
const NAV_LINKS = [
    { label: 'view E V V Timsheet Details For Payment', route: '/case/view-evv-timsheet-details-for-payment' }
  ];

export function CaseViewTimesheetDetailsForPaymentPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('case', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Case_viewTimesheetDetailsForPayment"}
...
export default CaseViewTimesheetDetailsForPaymentPage;

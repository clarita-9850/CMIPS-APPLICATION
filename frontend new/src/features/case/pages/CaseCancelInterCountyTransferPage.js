...
export function CaseCancelInterCountyTransferPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('case', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Case_cancelInterCountyTransfer"}
      title={"Cancel Inter County Transfer"}
      navLinks={NAV_LINKS}
...
export default CaseCancelInterCountyTransferPage;

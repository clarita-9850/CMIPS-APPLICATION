...
export function CaseCreateUHVLetterFollowupPage() {
  const navigate = useNavigate();
  const casesApi = getDomainApi('case');
  return (
    <UimPageLayout
      pageId={"Case_createUHVLetterFollowup"}
      title={"Create Follow-up Letter Entry:"}
      navLinks={NAV_LINKS}
      hidePlaceholderBanner={true}
    >
      <UimSection title={"Letter Details"}>
        <div className="uim-form-grid">
          <UimField label={"Letter Mailed Date"} />
        </div>
      </UimSection>
...
export default CaseCreateUHVLetterFollowupPage;

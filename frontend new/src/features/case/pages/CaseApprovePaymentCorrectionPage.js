...
export function CaseApprovePaymentCorrectionPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('case', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Case_approvePaymentCorrection"}
      title={"Approve Payment Correction:"}
      navLinks={NAV_LINKS}
...
export default CaseApprovePaymentCorrectionPage;

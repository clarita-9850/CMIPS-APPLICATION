...
const NAV_LINKS = [
    ...
    { label: 'view Travel Claim Snapshot', route: '/case/view-travel-claim-snapshot' }
  ];

export function CaseTravelClaimHistoryPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('case', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Case_travelClaimHistory"}
    ...
    >
      ...
      <div className="uim-action-bar">
        ...
        <button className="uim-btn uim-btn-primary" onClick={() => navigate('/case/view-travel-claim')}>View</button>
...
export default CaseTravelClaimHistoryPage;

...
export function CaseInactivateRecipFlexHoursPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('case', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Case_inactivateRecipFlexHours"}
      title={"Inactivate Recip Flex Hours"}
      navLinks={NAV_LINKS}
...
export default CaseInactivateRecipFlexHoursPage;

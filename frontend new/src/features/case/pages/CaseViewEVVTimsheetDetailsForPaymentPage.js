...
export function CaseViewEVVTimsheetDetailsForPaymentPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('case', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Case_viewEVVTimsheetDetailsForPayment"}
      title={"EVV Details:\\\\"}
      navLinks={NAV_LINKS}
...
export default CaseViewEVVTimsheetDetailsForPaymentPage;

...
const NAV_LINKS = [
    { label: 'reassessment Dashboard', route: '/organization/reassessment-dashboard' }
  ];

export function CaseSendTelehealthQuestionnairePage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('case', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Case_sendTelehealthQuestionnaire"}
      title={"Send Telehealth Questionnaire"}
      navLinks={NAV_LINKS}
...
export default CaseSendTelehealthQuestionnairePage;

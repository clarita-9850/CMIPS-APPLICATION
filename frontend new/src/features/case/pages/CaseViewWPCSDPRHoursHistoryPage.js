...
const NAV_LINKS = [
    { label: 'modify W P C S Differential P R Hours', route: '/case/modify-wpcs-differential-pr-hours' }
  ];

export function CaseViewWPCSDPRHoursHistoryPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('case', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Case_viewWPCSDPRHoursHistory"}
      title={"View WPCS Differential Pay Rate Hours History:"}
      navLinks={NAV_LINKS}
...
export default CaseViewWPCSDPRHoursHistoryPage;

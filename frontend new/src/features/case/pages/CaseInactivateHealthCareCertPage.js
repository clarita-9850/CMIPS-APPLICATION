...
export function CaseInactivateHealthCareCertPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('case', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Case_inactivateHealthCareCert"}
      title={"Inactivate Health Care Certification:"}
      navLinks={NAV_LINKS}
...
export default CaseInactivateHealthCareCertPage;

...
const NAV_LINKS = [
    { label: 'view Timesheet Snapshot', route: '/case/view-timesheet-snapshot' },
    { label: 'timesheet History', route: '/case/timesheet-history' }
  ];

export function CaseViewEVVTimsheetDetailsForHistoryPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('case', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Case_viewEVVTimsheetDetailsForHistory"}
      title={"EVV Details:\\\\"}
      navLinks={NAV_LINKS}
...
export default CaseViewEVVTimsheetDetailsForHistoryPage;

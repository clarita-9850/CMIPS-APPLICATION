import React from 'react';
import { useNavigate } from 'react-router-dom';
import { UimPageLayout } from '../../../shared/components';
import { UimSection }    from '../../../shared/components';
import { UimField }      from '../../../shared/components';
import { getDomainApi } from '../../../api/domainApi';

const NAV_LINKS = [];

export function CaseCreateProviderHoursPage() {
  const navigate = useNavigate();
  const casesApi = getDomainApi('case');
  return (
    <UimPageLayout
      pageId={"Case_createProviderHours"}
      title={"Create Provider Hours:"}
      navLinks={NAV_LINKS}
      hidePlaceholderBanner={true}
    >
      <UimSection title={"Provider Hours"}>
        <div className="uim-form-grid">
          <UimField label={"Provider Type"} />
          <UimField label={"Begin Date"} />
          <UimField label={"Pay Rate"} />
          <UimField label={"Assigned Hours (HH:MM)"} />
          <UimField label={"Provider Assigned Hours Form"} />
          <UimField label={"End Date"} />
          <UimField label={"Differential Pay Rate"} />
          <UimField label={"Max Differential Pay Rate Hours (HH:MM)"} />
        </div>
      </UimSection>
      <div className="uim-action-bar">
        <button className="uim-btn uim-btn-primary" onClick={() => { casesApi.create({}).then(() => { alert('Save successful'); navigate(-1); }).catch(err => alert('Save failed: ' + err.message)); }}>Save</button>
        <button className="uim-btn uim-btn-primary" onClick={() => alert('Action: Save &amp; New')}>Save &amp; New</button>
        <button className="uim-btn uim-btn-primary" onClick={() => navigate(-1)}>Cancel</button>
      </div>
    </UimPageLayout>
  );
}

export default CaseCreateProviderHoursPage;

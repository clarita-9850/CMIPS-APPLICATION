...
export function CaseRequestCashedWarrantCopyPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('case', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Case_requestCashedWarrantCopy"}
      title={"Request Cashed Warrant Copy"}
      navLinks={NAV_LINKS}
...
export default CaseRequestCashedWarrantCopyPage;

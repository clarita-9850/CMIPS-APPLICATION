...
export function CaseViewTravelClaimDetailsForPaymentPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('case', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Case_viewTravelClaimDetailsForPayment"}
      title={"View TravelClaim:\\\\"}
      navLinks={NAV_LINKS}
    ...
    >
      ...
      <UimSection title={"Details"}>
        <div className="uim-form-grid">
          ...
          <UimField label={"TravelClaim Number"} value={record && record['travelClaimNumber']} />
...
export default CaseViewTravelClaimDetailsForPaymentPage;

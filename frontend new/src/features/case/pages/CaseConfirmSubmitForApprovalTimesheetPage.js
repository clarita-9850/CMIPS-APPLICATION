...
export function CaseConfirmSubmitForApprovalTimesheetPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('case', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Case_confirmSubmitForApprovalTimesheet"}
    ...
    >
      ...
      <UimSection title={"Details"}>
        <div className="uim-form-grid">
          <UimField label={"Submit Timesheet for Approval"} value={record && record['submitTimesheetForApproval']} />
        </div>
      </UimSection>
...
export default CaseConfirmSubmitForApprovalTimesheetPage;

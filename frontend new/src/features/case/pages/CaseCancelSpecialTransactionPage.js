...
export function CaseCancelSpecialTransactionPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('case', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Case_cancelSpecialTransaction"}
      title={"Cancel Special Transaction:"}
      navLinks={NAV_LINKS}
...
export default CaseCancelSpecialTransactionPage;

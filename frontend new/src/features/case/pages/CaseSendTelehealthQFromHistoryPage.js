...
export function CaseSendTelehealthQFromHistoryPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('case', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Case_sendTelehealthQFromHistory"}
      title={"Send Telehealth Q From History"}
      navLinks={NAV_LINKS}
...
export default CaseSendTelehealthQFromHistoryPage;

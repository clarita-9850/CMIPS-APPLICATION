...
const NAV_LINKS = [
    { label: 'search Paid Hours', route: '/case/search-paid-hours' }
  ];

export function CaseViewWPCSServiceHoursPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('case', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Case_viewWPCSServiceHours"}
      title={"WPCS Service Hours:"}
      navLinks={NAV_LINKS}
...
export default CaseViewWPCSServiceHoursPage;

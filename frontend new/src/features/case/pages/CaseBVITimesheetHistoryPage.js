...
const NAV_LINKS = [
    ...
    { label: 'View Provider E T Reject Release', route: '/case/view-provider-et-reject-release' },
    { label: 'view B V I Timesheet History', route: '/case/view-bvi-timesheet-history' }
  ];

export function CaseBVITimesheetHistoryPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('case', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Case_BVITimesheetHistory"}
      title={"TTS Timesheet Release/Reject History:"}
      navLinks={NAV_LINKS}
...
export default CaseBVITimesheetHistoryPage;

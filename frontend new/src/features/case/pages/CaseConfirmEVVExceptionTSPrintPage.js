...
export function CaseConfirmEVVExceptionTSPrintPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('case', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Case_confirmEVVExceptionTSPrint"}
      title={"Confirm E V V Exception T S Print"}
      navLinks={NAV_LINKS}
...
export default CaseConfirmEVVExceptionTSPrintPage;

...
export function CaseViewApproveRejectTimesheetTabPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('case', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Case_viewApproveRejectTimesheetTab"}
      title={"View Approve Reject Timesheet Tab"}
      navLinks={NAV_LINKS}
...
export default CaseViewApproveRejectTimesheetTabPage;

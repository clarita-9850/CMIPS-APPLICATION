...
export function CaseConfirmApproveTimesheetPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('case', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Case_confirmApproveTimesheet"}
    ...
    >
      ...
      <UimSection title={"Details"}>
        <div className="uim-form-grid">
          <UimField label={"Approve Timesheet"} value={record && record['approveTimesheet']} />
        </div>
      </UimSection>
...
export default CaseConfirmApproveTimesheetPage;

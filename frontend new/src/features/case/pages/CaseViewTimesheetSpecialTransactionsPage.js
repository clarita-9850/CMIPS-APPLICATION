...
export function CaseViewTimesheetSpecialTransactionsPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('case', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Case_viewTimesheetSpecialTransactions"}
...
export default CaseViewTimesheetSpecialTransactionsPage;

...
export function CaseInactivateIHSSESPStatusPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('case', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Case_inactivateIHSSESPStatus"}
      title={"Inactivate Recipient IHSS ESP Registration:"}
      navLinks={NAV_LINKS}
...
export default CaseInactivateIHSSESPStatusPage;

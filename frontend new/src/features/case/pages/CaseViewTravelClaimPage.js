...
const NAV_LINKS = [
    ...
    { label: 'search Travel Claim', route: '/case/search-travel-claim' }
  ];

export function CaseViewTravelClaimPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('case', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Case_viewTravelClaim"}
...
export default CaseViewTravelClaimPage;

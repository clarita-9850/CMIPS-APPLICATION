...
export function CaseCancelCashedWarrantCopyRequestPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('case', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Case_cancelCashedWarrantCopyRequest"}
      title={"Cancel Cashed Warrant Copy Request"}
      navLinks={NAV_LINKS}
...
export default CaseCancelCashedWarrantCopyRequestPage;

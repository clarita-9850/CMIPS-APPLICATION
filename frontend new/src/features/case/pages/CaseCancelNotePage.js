...
export function CaseCancelNotePage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('case', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Case_cancelNote"}
      title={"Cancel Note"}
      navLinks={NAV_LINKS}
    ...
    >
      ...
      <div className="uim-action-bar">
        ...
        <button className="uim-btn uim-btn-primary" onClick={() => navigate('/case/view-note')}>No</button>
      </div>
    </UimPageLayout>
  );
}

export default CaseCancelNotePage;

...
export function CaseCancelVoidOrReissueRequestPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('case', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Case_cancelVoidOrReissueRequest"}
      title={"Cancel Void Or Reissue Request"}
      navLinks={NAV_LINKS}
...
export default CaseCancelVoidOrReissueRequestPage;

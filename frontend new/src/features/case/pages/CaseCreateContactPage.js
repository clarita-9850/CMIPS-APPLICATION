import React from 'react';
import { useNavigate } from 'react-router-dom';
import { UimPageLayout } from '../../../shared/components';
import { UimSection }    from '../../../shared/components';
import { UimField }      from '../../../shared/components';
import { getDomainApi } from '../../../api/domainApi';

const NAV_LINKS = [
    { label: 'list Contacts', route: '/case/list-contacts' }
  ];

export function CaseCreateContactPage() {
  const navigate = useNavigate();
  const casesApi = getDomainApi('case');
  return (
    <UimPageLayout
      pageId={"Case_createContact"}
      title={"Create Contact:"}
      navLinks={NAV_LINKS}
      hidePlaceholderBanner={true}
    >
      <UimSection title={"Mailing Address"}>
        <div className="uim-form-grid">
          <UimField label={"Name"} />
          <UimField label={"Spoken Language"} />
          <UimField label={"Email Address"} />
          <UimField label={"Type"} />
          <UimField label={"Relationship"} />
          <UimField label={"Type"} />
          <UimField label={"Mailing Address Type"} />
        </div>
      </UimSection>
      <UimSection title={"Phone Numbers"}>
        <div className="uim-form-grid">
          <UimField label={"Type"} />
          <UimField label={"Type"} />
          <UimField label={"Phone # 1"} />
          <UimField label={"Phone # 2"} />
          <UimField label={"Extension"} />
          <UimField label={"Extension"} />
          <UimField label={"Comments"} />
        </div>
      </UimSection>
      <div className="uim-action-bar">
        <button className="uim-btn uim-btn-primary" onClick={() => { casesApi.create({}).then(() => { alert('Save successful'); navigate(-1); }).catch(err => alert('Save failed: ' + err.message)); }}>Save</button>
        <button className="uim-btn uim-btn-primary" onClick={() => alert('Action: Save &amp; New')}>Save &amp; New</button>
        <button className="uim-btn uim-btn-primary" onClick={() => navigate(-1)}>Cancel</button>
      </div>
    </UimPageLayout>
  );
}

export default CaseCreateContactPage;

...
export function CaseCancelOverpaymentCollectionPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('case', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Case_cancelOverpaymentCollection"}
      title={"Cancel Overpayment Collection:"}
      navLinks={NAV_LINKS}
...
export default CaseCancelOverpaymentCollectionPage;

...
export function CaseCancelSpecialTransactionSubmissionPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('case', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Case_cancelSpecialTransactionSubmission"}
      title={"Cancel Special Transaction Submission"}
      navLinks={NAV_LINKS}
...
export default CaseCancelSpecialTransactionSubmissionPage;

...
const NAV_LINKS = [
    { label: 'view Warrant Replacement Details', route: '/payment/view-warrant-replacement-details' }
  ];

export function PaymentEnterWarrantReplacementPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('payment', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Payment_enterWarrantReplacement"}
      title={"Enter Warrant Replacement"}
      navLinks={NAV_LINKS}
...
export default PaymentEnterWarrantReplacementPage;

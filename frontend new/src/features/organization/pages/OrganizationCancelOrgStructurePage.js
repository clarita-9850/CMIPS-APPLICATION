...
const NAV_LINKS = [
    { label: 'list Org Structures', route: '/organization/list-org-structures' }
  ];

export function OrganizationCancelOrgStructurePage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('organization', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Organization_cancelOrgStructure"}
      title={"Cancel Org Structure"}
      navLinks={NAV_LINKS}
...
export default OrganizationCancelOrgStructurePage;

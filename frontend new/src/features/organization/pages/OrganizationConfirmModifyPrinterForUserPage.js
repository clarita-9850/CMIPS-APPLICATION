...
const NAV_LINKS = [
    { label: 'modify Printer For User', route: '/organization/modify-printer-for-user' }
  ];

export function OrganizationConfirmModifyPrinterForUserPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('organization', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Organization_confirmModifyPrinterForUser"}
      title={"Confirm Modify Printer For User"}
      navLinks={NAV_LINKS}
...
export default OrganizationConfirmModifyPrinterForUserPage;

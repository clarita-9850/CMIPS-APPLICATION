...
export function OrganizationResolveOrgUnitHomePage() {
  const { data, loading, error } = useDomainData('organization', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Organization_resolveOrgUnitHome"}
      title={"Resolve Org Unit Home"}
      navLinks={NAV_LINKS}
...
export default OrganizationResolveOrgUnitHomePage;

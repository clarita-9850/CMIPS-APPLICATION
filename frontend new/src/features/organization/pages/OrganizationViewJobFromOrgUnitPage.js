...
export function OrganizationViewJobFromOrgUnitPage() {
  const { data, loading, error } = useDomainData('organization', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Organization_viewJobFromOrgUnit"}
      title={"View Job From Org Unit"}
      navLinks={NAV_LINKS}
...
export default OrganizationViewJobFromOrgUnitPage;

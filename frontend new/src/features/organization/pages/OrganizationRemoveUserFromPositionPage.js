...
export function OrganizationRemoveUserFromPositionPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('organization', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Organization_removeUserFromPosition"}
      title={"Remove User From Position"}
      navLinks={NAV_LINKS}
...
export default OrganizationRemoveUserFromPositionPage;

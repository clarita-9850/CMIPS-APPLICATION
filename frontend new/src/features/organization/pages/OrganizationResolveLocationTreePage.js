...
export function OrganizationResolveLocationTreePage() {
  const { data, loading, error } = useDomainData('organization', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Organization_resolveLocationTree"}
      title={"Resolve Location Tree"}
      navLinks={NAV_LINKS}
...
export default OrganizationResolveLocationTreePage;

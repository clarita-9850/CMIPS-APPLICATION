...
export function OrganizationModifyJobFromViewPage() {
  const { data, loading, error } = useDomainData('organization', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Organization_modifyJobFromView"}
      title={"Modify Job From View"}
      navLinks={NAV_LINKS}
...
export default OrganizationModifyJobFromViewPage;

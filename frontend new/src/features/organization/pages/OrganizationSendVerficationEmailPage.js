...
const NAV_LINKS = [
    { label: 'enter Verfication Email', route: '/organization/enter-verfication-email' },
    { label: 'csp Verify Contact Info', route: '/organization/csp-verify-contact-info' }
  ];

export function OrganizationSendVerficationEmailPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('organization', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Organization_sendVerficationEmail"}
...
export default OrganizationSendVerficationEmailPage;

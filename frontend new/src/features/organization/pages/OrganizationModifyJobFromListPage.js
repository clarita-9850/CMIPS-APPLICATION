...
export function OrganizationModifyJobFromListPage() {
  const { data, loading, error } = useDomainData('organization', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Organization_modifyJobFromList"}
      title={"Modify Job From List"}
      navLinks={NAV_LINKS}
...
export default OrganizationModifyJobFromListPage;

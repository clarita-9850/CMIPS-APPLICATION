...
export function OrganizationModifyUserFromHomePage() {
  const { data, loading, error } = useDomainData('organization', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Organization_modifyUserFromHome"}
      title={"Modify User From Home"}
      navLinks={NAV_LINKS}
...
export default OrganizationModifyUserFromHomePage;

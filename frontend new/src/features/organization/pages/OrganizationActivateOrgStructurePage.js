...
export function OrganizationActivateOrgStructurePage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('organization', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Organization_activateOrgStructure"}
      title={"Activate Org Structure"}
      navLinks={NAV_LINKS}
...
export default OrganizationActivateOrgStructurePage;

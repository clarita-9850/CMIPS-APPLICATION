...
export function OrganizationCloneOrgStructurePage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('organization', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Organization_cloneOrgStructure"}
      title={"Clone Org Structure"}
      navLinks={NAV_LINKS}
...
export default OrganizationCloneOrgStructurePage;

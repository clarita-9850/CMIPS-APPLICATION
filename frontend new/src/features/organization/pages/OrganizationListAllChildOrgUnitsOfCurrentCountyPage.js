...
export function OrganizationListAllChildOrgUnitsOfCurrentCountyPage() {
  const { data, loading, error } = useDomainData('organization', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Organization_listAllChildOrgUnitsOfCurrentCounty"}
...
export default OrganizationListAllChildOrgUnitsOfCurrentCountyPage;

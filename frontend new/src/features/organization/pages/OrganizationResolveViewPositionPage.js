...
export function OrganizationResolveViewPositionPage() {
  const { data, loading, error } = useDomainData('organization', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Organization_resolveViewPosition"}
      title={"Resolve View Position"}
      navLinks={NAV_LINKS}
...
export default OrganizationResolveViewPositionPage;

...
export function OrganizationResolveLocationHomePage() {
  const { data, loading, error } = useDomainData('organization', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Organization_resolveLocationHome"}
      title={"Resolve Location Home"}
      navLinks={NAV_LINKS}
...
export default OrganizationResolveLocationHomePage;

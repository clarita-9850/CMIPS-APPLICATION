...
export function CountyPrintContractExpenditureReportPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('county', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"County_printContractExpenditureReport"}
      title={"Print Contract Expenditure Report"}
      navLinks={NAV_LINKS}
...
export default CountyPrintContractExpenditureReportPage;

...
export function RegionalAdminActivateLocationStructurePage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('county', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"RegionalAdmin_activateLocationStructure"}
      title={"Activate Location Structure"}
      navLinks={NAV_LINKS}
...
export default RegionalAdminActivateLocationStructurePage;

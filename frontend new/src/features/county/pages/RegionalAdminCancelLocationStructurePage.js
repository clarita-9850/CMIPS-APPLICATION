...
export function RegionalAdminCancelLocationStructurePage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('county', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"RegionalAdmin_cancelLocationStructure"}
      title={"Cancel Location Structure"}
      navLinks={NAV_LINKS}
...
export default RegionalAdminCancelLocationStructurePage;

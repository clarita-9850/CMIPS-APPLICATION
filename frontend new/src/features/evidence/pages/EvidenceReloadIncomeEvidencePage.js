...
export function EvidenceReloadIncomeEvidencePage() {
  const { data, loading, error } = useDomainData('evidence', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Evidence_reloadIncomeEvidence"}
      title={"Reload Income Evidence"}
      navLinks={NAV_LINKS}
...
export default EvidenceReloadIncomeEvidencePage;

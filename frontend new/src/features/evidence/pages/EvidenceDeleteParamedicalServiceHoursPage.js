...
export function EvidenceDeleteParamedicalServiceHoursPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('evidence', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Evidence_deleteParamedicalServiceHours"}
      title={"Delete Paramedical Service Hours"}
      navLinks={NAV_LINKS}
...
export default EvidenceDeleteParamedicalServiceHoursPage;

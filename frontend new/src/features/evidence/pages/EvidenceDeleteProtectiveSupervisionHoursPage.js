...
export function EvidenceDeleteProtectiveSupervisionHoursPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('evidence', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Evidence_deleteProtectiveSupervisionHours"}
      title={"Delete Protective Supervision Hours"}
      navLinks={NAV_LINKS}
...
export default EvidenceDeleteProtectiveSupervisionHoursPage;

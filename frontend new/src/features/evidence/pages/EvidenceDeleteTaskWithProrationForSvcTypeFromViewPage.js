...
export function EvidenceDeleteTaskWithProrationForSvcTypeFromViewPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('evidence', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Evidence_deleteTaskWithProrationForSvcTypeFromView"}
      title={"Delete Task With Proration For Svc Type From View"}
      navLinks={NAV_LINKS}
...
export default EvidenceDeleteTaskWithProrationForSvcTypeFromViewPage;

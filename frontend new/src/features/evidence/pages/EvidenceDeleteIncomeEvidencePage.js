...
export function EvidenceDeleteIncomeEvidencePage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('evidence', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Evidence_deleteIncomeEvidence"}
      title={"Delete Income Evidence"}
      navLinks={NAV_LINKS}
...
export default EvidenceDeleteIncomeEvidencePage;

...
export function EvidenceDeletePendingEvidencePage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('evidence', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Evidence_deletePendingEvidence"}
      title={"Delete Pending Evidence"}
      navLinks={NAV_LINKS}
...
export default EvidenceDeletePendingEvidencePage;

...
export function EvidenceViewServiceTypeWithoutProrationPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('evidence', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Evidence_viewServiceTypeWithoutProration"}
...
export default EvidenceViewServiceTypeWithoutProrationPage;

...
export function EvidenceViewTeachAndDemoServicePage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('evidence', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Evidence_viewTeachAndDemoService"}
    ...
    >
      ...
      <UimSection title={"Details"}>
        <div className="uim-form-grid">
          ...
          <UimField label={"Number Of Months"} value={record && record['numberOfMonths']} />
...
export default EvidenceViewTeachAndDemoServicePage;

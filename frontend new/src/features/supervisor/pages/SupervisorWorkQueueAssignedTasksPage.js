...
const NAV_LINKS = [
    { label: 'reserve Work Queue Tasks To User', route: '/supervisor/reserve-work-queue-tasks-to-user' },
    { label: 'view Task', route: '/supervisor/view-task' }
  ];

export function SupervisorWorkQueueAssignedTasksPage() {
  const { data, loading, error } = useDomainData('supervisor', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Supervisor_workQueueAssignedTasks"}
...
export default SupervisorWorkQueueAssignedTasksPage;

...
const NAV_LINKS = [
    { label: 'redirect Tasks To Users', route: '/supervisor/redirect-tasks-to-users' }
  ];

export function SupervisorClearTaskRedirectionPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('supervisor', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Supervisor_clearTaskRedirection"}
      title={"Clear Task Redirection"}
      navLinks={NAV_LINKS}
...
export default SupervisorClearTaskRedirectionPage;

...
export function SupervisorUserWorkQueuesPage() {
  const { data, loading, error } = useDomainData('supervisor', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Supervisor_userWorkQueues"}
      title={"Work Queues"}
      navLinks={NAV_LINKS}
    ...
    >
      ...
      <UimSection title={"Details"}>
        <div className="uim-form-grid">
          ...
          <UimField label={"Tasks in Queue"} value={record && record['tasksInQueue']} />
        </div>
      </UimSection>
...
export default SupervisorUserWorkQueuesPage;

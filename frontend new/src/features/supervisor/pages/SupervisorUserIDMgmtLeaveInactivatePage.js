...
export function SupervisorUserIDMgmtLeaveInactivatePage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('supervisor', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Supervisor_userIDMgmtLeaveInactivate"}
      title={"Inactivate User ID Management - Leave"}
      navLinks={NAV_LINKS}
...
export default SupervisorUserIDMgmtLeaveInactivatePage;

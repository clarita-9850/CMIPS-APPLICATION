...
export function SupervisorUserIDMgmtExemptInactivatePage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('supervisor', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Supervisor_userIDMgmtExemptInactivate"}
      title={"Inactivate User ID Management - Deactivation Exemption"}
      navLinks={NAV_LINKS}
...
export default SupervisorUserIDMgmtExemptInactivatePage;

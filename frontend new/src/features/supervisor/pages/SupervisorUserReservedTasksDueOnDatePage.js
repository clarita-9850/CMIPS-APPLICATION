...
export function SupervisorUserReservedTasksDueOnDatePage() {
  const { data, loading, error } = useDomainData('supervisor', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Supervisor_userReservedTasksDueOnDate"}
...
export default SupervisorUserReservedTasksDueOnDatePage;

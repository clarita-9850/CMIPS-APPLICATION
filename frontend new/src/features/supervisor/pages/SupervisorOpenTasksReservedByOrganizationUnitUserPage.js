...
export function SupervisorOpenTasksReservedByOrganizationUnitUserPage() {
  const { data, loading, error } = useDomainData('supervisor', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Supervisor_openTasksReservedByOrganizationUnitUser"}
      title={"Organization Unit Reserved Tasks"}
      navLinks={NAV_LINKS}
    ...
    >
      ...
      <UimSection title={"Details"}>
        <div className="uim-form-grid">
          ...
          <UimField label={"Reserved"} value={record && record['reserved']} />
...
export default SupervisorOpenTasksReservedByOrganizationUnitUserPage;

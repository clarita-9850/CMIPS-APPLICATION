...
export function SupervisorOrgUnitsNavigationPage() {
  const { data, loading, error } = useDomainData('supervisor', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Supervisor_orgUnitsNavigation"}
      title={"Org Units Navigation"}
      navLinks={NAV_LINKS}
...
export default SupervisorOrgUnitsNavigationPage;

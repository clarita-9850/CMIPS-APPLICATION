...
export function SupervisorTaskRedirectionHistoryPage() {
  const { data, loading, error } = useDomainData('supervisor', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Supervisor_taskRedirectionHistory"}
      title={"Task Redirection History"}
      navLinks={NAV_LINKS}
...
export default SupervisorTaskRedirectionHistoryPage;

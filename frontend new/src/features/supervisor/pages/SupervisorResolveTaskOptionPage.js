...
export function SupervisorResolveTaskOptionPage() {
  const { data, loading, error } = useDomainData('supervisor', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Supervisor_resolveTaskOption"}
      title={"Resolve Task Option"}
      navLinks={NAV_LINKS}
...
export default SupervisorResolveTaskOptionPage;

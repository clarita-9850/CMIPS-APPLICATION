...
const NAV_LINKS = [
    { label: 'task Allocation Blocking', route: '/supervisor/task-allocation-blocking' }
  ];

export function SupervisorClearTaskAllocationBlockPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('supervisor', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Supervisor_clearTaskAllocationBlock"}
      title={"Clear Task Allocation Block"}
      navLinks={NAV_LINKS}
...
export default SupervisorClearTaskAllocationBlockPage;

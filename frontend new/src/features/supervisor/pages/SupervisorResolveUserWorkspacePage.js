...
export function SupervisorResolveUserWorkspacePage() {
  const { data, loading, error } = useDomainData('supervisor', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Supervisor_resolveUserWorkspace"}
      title={"Resolve User Workspace"}
      navLinks={NAV_LINKS}
...
export default SupervisorResolveUserWorkspacePage;

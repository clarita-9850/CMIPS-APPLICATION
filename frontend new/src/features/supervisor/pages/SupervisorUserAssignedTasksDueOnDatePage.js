...
export function SupervisorUserAssignedTasksDueOnDatePage() {
  const { data, loading, error } = useDomainData('supervisor', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Supervisor_userAssignedTasksDueOnDate"}
...
export default SupervisorUserAssignedTasksDueOnDatePage;

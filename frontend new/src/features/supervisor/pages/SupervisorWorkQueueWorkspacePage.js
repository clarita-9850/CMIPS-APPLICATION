...
export function SupervisorWorkQueueWorkspacePage() {
  const { data, loading, error } = useDomainData('supervisor', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Supervisor_workQueueWorkspace"}
      title={"Home"}
      navLinks={NAV_LINKS}
    ...
    >
      ...
      <UimSection title={"User Subscribers"}>
        <UimTable
...
export default SupervisorWorkQueueWorkspacePage;

...
export function SupervisorWorkQueueSchedulePage() {
  const { data, loading, error } = useDomainData('supervisor', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Supervisor_workQueueSchedule"}
      title={"Work Queue Schedule"}
      navLinks={NAV_LINKS}
...
export default SupervisorWorkQueueSchedulePage;

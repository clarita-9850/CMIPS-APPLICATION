...
export function HomemakerApproveHomemakerTimesheetPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('homemaker', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Homemaker_approveHomemakerTimesheet"}
      title={"Approve Homemaker Timesheet"}
      navLinks={NAV_LINKS}
...
export default HomemakerApproveHomemakerTimesheetPage;

...
const NAV_LINKS = [
    { label: 'view Inactivate Record', route: '/homemaker/view-inactivate-record' },
    { label: 'list Records', route: '/homemaker/list-records' }
  ];

export function WorkWeekAgreementInactivatelistRecordsPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('homemaker', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"WorkWeekAgreement_inactivatelistRecords"}
      title={"Inactive Provider Workweek Agreement History:"}
      navLinks={NAV_LINKS}
    ...
    >
      ...
      <div className="uim-action-bar">
        <button className="uim-btn uim-btn-primary" onClick={() => navigate('/homemaker/view-inactivate-record')}>View</button>
...
export default WorkWeekAgreementInactivatelistRecordsPage;

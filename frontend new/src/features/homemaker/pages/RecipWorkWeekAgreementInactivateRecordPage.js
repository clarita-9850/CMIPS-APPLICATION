...
export function RecipWorkWeekAgreementInactivateRecordPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('homemaker', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"RecipWorkWeekAgreement_inactivateRecord"}
      title={"Inactivate Record"}
      navLinks={NAV_LINKS}
...
export default RecipWorkWeekAgreementInactivateRecordPage;

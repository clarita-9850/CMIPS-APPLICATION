...
export function HomemakerInactivateHomemakerTimesheetPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('homemaker', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Homemaker_inactivateHomemakerTimesheet"}
      title={"Inactivate Homemaker Timesheet"}
      navLinks={NAV_LINKS}
...
export default HomemakerInactivateHomemakerTimesheetPage;

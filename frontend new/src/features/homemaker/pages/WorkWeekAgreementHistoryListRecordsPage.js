...
export function WorkWeekAgreementHistoryListRecordsPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useDomainData('homemaker', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"WorkWeekAgreementHistory_listRecords"}
      title={"Provider Workweek Agreement History:"}
      navLinks={NAV_LINKS}
...
export default WorkWeekAgreementHistoryListRecordsPage;

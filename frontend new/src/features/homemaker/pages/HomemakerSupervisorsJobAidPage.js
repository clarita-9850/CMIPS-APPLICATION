...
export function HomemakerSupervisorsJobAidPage() {
  const { data, loading, error } = useDomainData('homemaker', 'list');
  const record = Array.isArray(data) ? data[0] : data;
  return (
    <UimPageLayout
      pageId={"Homemaker_Supervisors_Job_Aid"}
      title={"Supervisors Job Aid"}
      navLinks={NAV_LINKS}
...
export default HomemakerSupervisorsJobAidPage;

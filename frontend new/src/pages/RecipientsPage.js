...
/**
 * Find a Person — General person search (DSD-compliant layout)
 *
 * Same layout as PersonSearchReferralPage / PersonSearchApplicationPage but
 * without any "Continue Referral" or "Continue Application" button.
 * Clicking a name navigates to the Person Home (RecipientDetailPage).
 */
export const RecipientsPage = () => {
  ...
  const doSearch = async (pageNum = 0) => {
    ...
    try {
      ...
      console.error('[FindAPerson] Search failed:', err);

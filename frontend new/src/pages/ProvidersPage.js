...
export const ProvidersPage = () => {
  ...
  const doSearch = () => {
    ...
    console.log('[ProvidersPage] doSearch, url:', url);
    http.get(url)
      .then(res => {
        console.log('[ProvidersPage] Search response:', res);
        const d = res?.data;
        const resultArray = Array.isArray(d) ? d : (d?.content || d?.items || []);
        console.log('[ProvidersPage] Results array:', resultArray);
        setResults(resultArray);
      })
      .catch(err => {
        console.error('[ProvidersPage] Search error:', err);
        setResults([]);
      })

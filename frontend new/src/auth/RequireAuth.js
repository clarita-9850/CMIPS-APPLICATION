 ...
 *
 * Requires Keycloak authentication or falls back to localStorage check
 */
export const RequireAuth = ({ children }) => {
  ...
  useEffect(() => {
    // If not loading and not authenticated, trigger login (once)
    if (!loading && !authenticated && keycloak && !loginTriggered.current) {
      console.log('User not authenticated, redirecting to login...');
      loginTriggered.current = true;
      login();
    }
    // Reset the guard when the user becomes authenticated
    if (authenticated) {

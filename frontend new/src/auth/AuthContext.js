...
export const AuthProvider = ({ children, keycloakInstance, noAuthMode = false }) => {
  ...
  // In Keycloak mode, start loading=true so RequireAuth shows the loading screen
  // until the useEffect below has synced keycloakInstance.authenticated into React state.
  // Without this, RequireAuth's useEffect fires before ours (children effects run first)
  // and calls login() while authenticated is still at its initial false value — causing
  // an infinite redirect loop (Keycloak SSO → redirect back → login() again).
  const [loading, setLoading] = useState(!noAuthMode);
 ...
 *
 * Checks authentication and optionally required roles before rendering children
 */

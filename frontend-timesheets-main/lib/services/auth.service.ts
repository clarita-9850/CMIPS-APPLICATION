...
function parseUserInfoFromToken(token: string): UserInfo {
  try {
    ...
    // Extract role from client roles (cmips-app) or realm roles
    ...
    if (typeof window !== 'undefined') {
      console.log('🔍 JWT Payload structure:', {
        ...
        hasCmipsApp: !!payload.resource_access?.['cmips-app'],
        cmipsAppKeys: payload.resource_access?.['cmips-app'] ? Object.keys(payload.resource_access['cmips-app']) : [],
        clientRoles: payload.resource_access?.['cmips-app']?.roles,
    ...
    if (payload.resource_access?.['cmips-app']?.roles) {
      const clientRoles = payload.resource_access['cmips-app'].roles;
    ...
    if (uniqueRoles.includes('ADMIN')) {
      ...
      // Try to find roles in ANY client (not just cmips-app)
    ...
    if (!userInfo.role || userInfo.role.trim() === '') {
      ...
      // Don't throw - return empty role so UI can handle it
    } else {
...
export const authService = {
  /**
   * Login with username and password - calls Keycloak directly
   */
  async login(credentials: LoginRequest): Promise<AuthResponse> {
    // Use API Gateway for authentication - no direct Keycloak access needed
    // Always use localhost:8090 for client-side requests (browser cannot resolve api-gateway hostname)
    ...
    try {
      // Send JSON request to gateway - gateway will transform to Keycloak format

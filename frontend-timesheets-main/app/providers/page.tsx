...
export default function ProviderManagementPage() {
  ...
  const fetchProviders = useCallback(async (page: number = 0) => {
    try {
      ...
      const response = await apiClient.get(`/api/providers?page=${page}&size=${pageSize}&countyCode=${countyCode}`);
      const data = response.data;
      const providerData = data.content || [];
      ...
      const pendingCount = providerData.filter((p: Provider) => !p.backgroundCheckCompleted).length;
      const pageTotal = providerData.length || 1;

      setStats({
        total: data.totalElements || 0,
        active: Math.round((activeCount / pageTotal) * (data.totalElements || 0)),
        onLeave: Math.round((onLeaveCount / pageTotal) * (data.totalElements || 0)),
        pendingCertification: Math.round((pendingCount / pageTotal) * (data.totalElements || 0))
      });
    } catch (err) {
  ...
  const handleSearch = async () => {
    try {
      ...
      // Note: status filter not supported by backend search

      const response = await apiClient.get(`/api/providers/search?${params.toString()}`);
        ...
        <div className={styles.content}>
          ...
          <div className={styles.quickActions}>
            ...
            <div className={styles.quickActionsBody}>
              ...
              <button className={styles.quickActionBtn} onClick={() => {
                setFilteredProviders(providers.filter(p => p.sickLeaveEligibleDate));
              }}>
                    ...
                    <tbody>
                      {filteredProviders.map((provider) => (
                        <tr key={provider.id}>
                          ...
                          <td>
                            {provider.sickLeaveEligibleDate ? (
                              <span className={styles.sickLeaveHours}>
                                {provider.sickLeaveAccruedHours || 0} hrs
                              </span>
                            ) : (

...
public interface ProviderAssignmentRepository extends JpaRepository<ProviderAssignmentEntity, Long> {
    ...
    // PVM-07: Find active assignments with end dates before a given date
    @Query("SELECT pa FROM ProviderAssignmentEntity pa WHERE pa.status = :status AND pa.endDate IS NOT NULL AND pa.endDate <= :endDate")
    List<ProviderAssignmentEntity> findByStatusAndEndDateBefore(@Param("status") AssignmentStatus status, @Param("endDate") LocalDate endDate);
}

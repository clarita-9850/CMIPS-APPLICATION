...
public class KeycloakPolicyEvaluationService {
    ...
    public void refreshPermissionCache() {
        ...
        try {
            ...
            // 2b. Fetch all realm roles -> build roleId -> roleName map (for resolving UUIDs in policies)
            Map<String, String> roleIdToName = new HashMap<>();
            try {
                List<Map<String, Object>> allRoles = keycloakAdminService.getAllRoles();
                for (Map<String, Object> role : allRoles) {
                    String id = (String) role.get("id");
                    String name = (String) role.get("name");
                    if (id != null && name != null) {
                        roleIdToName.put(id, name);
                    }
                }
                logger.debug("Loaded {} realm roles for UUID resolution", roleIdToName.size());
            } catch (Exception e) {
                logger.warn("Failed to load realm roles for UUID resolution: {}", e.getMessage());
            }
            ...
            for (Map<String, Object> policy : rolePolicies) {
                ...
                // Extract role name from policy — resolve role UUIDs to actual role names
                String roleName = extractRoleNameFromPolicy(policy, policyName, roleIdToName);
                if (roleName != null) {
            ...
            for (Map<String, Object> permission : permissions) {
                ...
                // API fallback: if resources/scopes still empty, fetch them directly from Keycloak
                if (resourceNames.isEmpty()) {
                    try {
                        List<Map<String, Object>> permResources = keycloakAdminService.getPermissionResources(permissionId);
                        for (Map<String, Object> r : permResources) {
                            ...
                            if (name != null) resourceNames.add(name);
                                ...
                                if (id != null) {
                                    ...
                                    if (resolved != null) resourceNames.add(resolved);
                                }
                            }
                        }
                    } catch (Exception e) {
                        logger.warn("Failed to fetch resources for permission {}: {}", permissionId, e.getMessage());
                    }
                }
                if (scopeNames.isEmpty()) {
                    try {
                        ...
                        logger.warn("Failed to fetch scopes for permission {}: {}", permissionId, e.getMessage());
                    }
                }
    ...
    private String extractRoleNameFromPolicy(Map<String, Object> policy, String policyName, Map<String, String> roleIdToName) {
        // Try naming convention: "Policy-ROLENAME"
        if (policyName.startsWith("Policy-")) {
            return policyName.substring("Policy-".length());
        }

        // Try to extract role from the roles array in the policy
        ...
        if (rolesObj instanceof List) {
            ...
            if (!roles.isEmpty()) {
                ...
                String roleId = (String) firstRole.get("id");
                if (roleId != null && roleIdToName.containsKey(roleId)) {
                    return roleIdToName.get(roleId);
                }
            }
        }

        // Try config field (Keycloak 23+ puts role info in config.roles as JSON string)
        ...
        if (configObj instanceof Map) {
            ...
            if (configRoles instanceof String) {
                try {
                    ...
                    List<Map<String, Object>> parsedRoles = mapper.readValue((String) configRoles,
                        mapper.getTypeFactory().constructCollectionType(List.class, Map.class));
                    if (!parsedRoles.isEmpty()) {
                        String roleId = (String) parsedRoles.get(0).get("id");
                        if (roleId != null && roleIdToName.containsKey(roleId)) {
                            return roleIdToName.get(roleId);
                        }
                    }
                } catch (Exception e) {
                    logger.debug("Failed to parse config.roles for policy {}: {}", policyName, e.getMessage());
                }
            }
        }
        ...
        if (roleNames.isEmpty()) {
            try {
                ...
                for (Map<String, Object> assocPolicy : associatedPolicies) {
                    ...
                    String assocPolicyType = (String) assocPolicy.get("type");

                    // Check in our policyId map first (role policies)
                    ...
                    } else if ("aggregate".equals(assocPolicyType) && assocPolicyId != null) {
                        // Aggregate policy — resolve its sub-policies to find role policies
                        try {
                            List<Map<String, Object>> subPolicies = keycloakAdminService.getPermissionAssociatedPolicies(assocPolicyId);
                            for (Map<String, Object> subPolicy : subPolicies) {
                                String subId = (String) subPolicy.get("id");
                                if (subId != null && policyIdToRoleName.containsKey(subId)) {
                                    roleNames.add(policyIdToRoleName.get(subId));
                                }
                            }
                        } catch (Exception ae) {
                            logger.debug("Failed to resolve aggregate policy {}: {}", assocPolicyId, ae.getMessage());
                        }
                    }
                }
            } catch (Exception e) {
